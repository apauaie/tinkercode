...
Blockly.Arduino.eureqa_rgb = function() {
 ...
 Blockly.Arduino.definitions_['define_rgb_'] = 'uint8_t* rgb_arr = NULL;uint32_t t_f;\n';
  Blockly.Arduino.definitions_['define_rgb_function'+DIGITAL_PIN] = 'void ArGiBi_'+DIGITAL_PIN+'(int r, int g, int b){uint8_t *w = &rgb_arr[0]; *w++ = g;  *w++ = r;*w = b;  while((micros() - t_f) < 50L);cli(); volatile uint8_t *p = rgb_arr,val  = *p++,high = '+PORT+' |  _BV('+PORT_PIN+'),low  = '+PORT+' & ~_BV('+PORT_PIN+'),nbits= 8;asm volatile("nextbit_'+DIGITAL_PIN+':\\n\\t""sbi  %0, %1\\n\\t""sbrc %4, 7\\n\\t""mov  %6, %3\\n\\t""dec  %5\\n\\t""nop\\n\\t""st   %a2, %6\\n\\t""mov  %6, %7\\n\\t""breq nextbyte_'+DIGITAL_PIN+'\\n\\t""rol  %4\\n\\t""rjmp .+0\\n\\t""cbi   %0, %1\\n\\t""rjmp .+0\\n\\t""nop\\n\\t""rjmp nextbit_'+DIGITAL_PIN+'\\n\\t""nextbyte_'+DIGITAL_PIN+':\\n\\t""ldi  %5, 8\\n\\t""ld   %4, %a8+\\n\\t""cbi   %0, %1\\n\\t""rjmp .+0\\n\\t""nop\\n\\t" "dec %9\\n\\t""brne nextbit_'+DIGITAL_PIN+'\\n\\t"::"I" (_SFR_IO_ADDR('+PORT+')),"I" ('+PORT_PIN+'),"e" (&'+PORT+'),"r" (high),"r" (val),"r" (nbits),"r" (low),"r" (low),"e" (p),"w" (3));sei(); t_f = micros();};\n';
  ...
  var code = 'ArGiBi_'+DIGITAL_PIN+'('+r+','+g+','+b+');\n';
  return code;
};


Blockly.Arduino.eureqa_audio = function() {
	  var audiofile = this.getFieldValue('audioindex');
	  var port = this.getFieldValue('port');
   var spliter = port.split(',');
	  var tx= spliter[0];
	  var rx= spliter[1];
 ...
 Blockly.Arduino.definitions_['define_audio_'] = '#include "Arduino.h"\n#include "SoftwareSerial.h"\n#include "DFRobotDFPlayerMini.h"\nSoftwareSerial audioSerial('+rx+','+tx+');\nDFRobotDFPlayerMini myDFPlayer;\n';
 
  Blockly.Arduino.setups_['setup_audio'] = 'audioSerial.begin(9600);\n myDFPlayer.begin(audioSerial);\n';

  var code = 'myDFPlayer.play('+audiofile+');\n';
  return code;
};
...
Blockly.Arduino.eureqa_motor_steer = function() {
  ...
  if (dropdown_dir=="forward") {left_dir="HIGH";right_dir="LOW";left_speed=value_speed;right_speed=value_speed;}
 else if (dropdown_dir=="backward") {left_dir="LOW";right_dir="HIGH";left_speed=value_speed;right_speed=value_speed;}
  else if (dropdown_dir=="right") {left_dir="HIGH";right_dir="HIGH";left_speed=Math.round(value_speed/2);right_speed=value_speed;}
  else if (dropdown_dir=="left") {left_dir="LOW";right_dir="LOW";left_speed=value_speed;right_speed=Math.round(value_speed/2);}
    else if (dropdown_dir=="stop") {left_dir="LOW";right_dir="LOW";left_speed=0;right_speed=0;}
  else {left_dir='HIGH';right_dir='LOW';left_speed=value_speed;right_speed=value_speed;}
...
if (dropdown_dirl=="forward") {left_dir="HIGH";}
 ...
 if (dropdown_dirr=="forward") {right_dir="LOW";}
 else {right_dir="HIGH";}
...
Blockly.Arduino.eureqa_bluetooth_ready = function() {



Blockly.Arduino.definitions_['define_eureqa_bluetooth'] = "#include <SoftwareSerial.h>\n SoftwareSerial bluetooth(2, 4); \n char BTdata; // variable for bluetooth";


     Blockly.Arduino.setups_['eureqa_bluetooth']='  bluetooth.begin(9600);\n';
          // get the device ID, this is just a test to see if we're connected
...
Blockly.Arduino.eureqa_bluetooth_data = function() {
...
Blockly.Arduino.eureqa_siren = function() {

Blockly.Arduino.definitions_['define_eureqa_siren'] = "int speakerPin=10;\n\nvoid siren(){ for(int hz = 440; hz < 1000; hz++){\ntone(speakerPin, hz, 50);\ndelay(5);\n}\nnoTone(speakerPin);\nfor(int hz = 1000; hz > 440; hz--){\ntone(speakerPin, hz, 50);\ndelay(5);\n}\nnoTone(speakerPin);}\n";


     Blockly.Arduino.setups_['eureqa_siren']='pinMode(speakerPin,OUTPUT);';
          // get the device ID, this is just a test to see if we're connected
